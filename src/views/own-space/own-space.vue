                ...
                >
                    ...
                    <FormItem label="用户姓名：">
                        <div style="display:inline-block;width:300px;">
                            <Input v-model="userForm.user_true_name"></Input>
                        </div>
                    </FormItem>
        ...
        methods: {
            ...
            saveEdit () {
                ...
                /*
                 this.$refs['userForm'].validate((valid) => {
                 if (valid) {
                 if (this.phoneHasChanged && this.userForm.user_phone !== this.initPhone) { // 手机号码修改过了而且修改之后的手机号和原来的不一样
            ...
            saveEditPass () {
                ...
                this.$refs['editPasswordForm'].validate((valid) => {
                    if (valid) {
                        ...
                        // you can write ajax request here
                        api.Post('UserChangePwdApi', {
                            old_pwd: util.secret(vm.editPasswordForm.oldPass),
                            pwd: util.secret(vm.editPasswordForm.newPass)
                        }, function (res) {
            ...
            init () {
                let vm = this;
                api.Post('UserGetApi', {}, function (res) {
                    if (res.code === 0) {
                        vm.userForm.user_code = res.user_code;
                        vm.userForm.user_name = res.user_name;
                        vm.userForm.user_true_name = res.user_true_name ? res.user_true_name : res.user_name;
                        vm.userForm.user_phone = res.user_phone;
                        vm.initPhone = res.user_phone;
                        vm.userForm.company = '';
                        vm.userForm.department = '';
                    } else {
                        vm.$Notice.warning({
                            title: '错误',
                            desc: res.msg
            ...
            cancelInputCodeBox () {
                ...
                this.userForm.user_phone = this.user_phone;
            },
            submitCode () {
                let vm = this;
            ...
            saveInfoAjax () {
                ...
                api.Post('UserEditApi', {
                    user_true_name: this.userForm.user_true_name,
